...
class HeaderComponent extends React.Component<IHeaderProps, IHeaderState> {
    ...
    private handleScroll() {
        ...
        this.scrollTimer = setTimeout(() => {
            this.setState({
                unTransparent:
                    window.scrollY > WIN_SCROLLY || !/^(\/|\/template)$/.test(location.pathname)
            });
        }, 100);
    }
    ...
    public render(): React.ReactNode {
        ...
        return (
            <div className={`${unTrans ? style.headerUnTransparent : ''} ${style.headerContent}`}>
                <div className={style.headerLogo}>
                    <span style={{ color: unTrans ? '#06c' : '' }}>D</span>
                    <span style={{ color: unTrans ? '#f60' : '' }}>igital</span>
                </div>
                <MenuCompt
                    menuList={MenuList}
